...
def seed_league(db: Session, yahoo_id: int = 528886, name: str | None = None) -> League:
    ...
    # Compare on string form to match underlying column type and avoid
    # duplicate inserts when tests pass integers.
    league = db.execute(
        select(League).where(League.yahoo_league_id == str(yahoo_id))
    ).scalar_one_or_none()
    if league is None:
